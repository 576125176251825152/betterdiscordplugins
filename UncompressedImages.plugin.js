  ...
  start() {
    (function () {
      ...
      function transformUrl(raw) {
        ...
        if (
          !raw.includes("media.discordapp.net") &&
      ...
      function transformSrcset(srcset) {
        ...
        return srcset
          ...
          .map((part) => {
            ...
            if (
              isVideoExtension(urlPart) ||
              urlCache.get(urlPart) === "__VIDEO__"
            )
              return trimmed;
      ...
      const origSrcsetDesc =
        Object.getOwnPropertyDescriptor(imgProto, "srcset") || {};
      ...
      function applyTransformedSrc(img, originalString) {
        try {
          ...
          if (
            isVideoExtension(originalString) ||
            urlCache.get(originalString) === "__VIDEO__"
          )
            return false;

          const cached =
            urlCache.get(originalString) || transformUrl(originalString);
      ...
      function setAttributeWrapper(name, value) {
        try {
          ...
          if (lower === "src" && this instanceof HTMLImageElement) {
            ...
            const transformed = urlCache.has(s)
              ? urlCache.get(s)
              : transformUrl(s);
        ...
        Object.defineProperty(imgProto, "src", {
          ...
          set: function (val) {
            try {
              ...
              if (
                !s.includes("media.discordapp.net") ||
      ...
      function processImg(img) {
        try {
          ...
          // Video filter.
          if (
            isVideoExtension(img.src) ||
            urlCache.get(img.src) === "__VIDEO__"
          )
            return;
          ...
          if (attrSrc) {
            ...
            if (
              propSrc.includes("media.discordapp.net") &&
              propSrc.includes("/attachments/")
            ) {
