...
module.exports = ((_) => {
  ...
  return !window.BDFDB_Global ||
    ...
    : (([Plugin, BDFDB]) => {
        ...
        const ReadAllButtonComponent = class ReadAllButton
          extends BdApi.React.Component
        {
          clearClick() {
            if (_this.settings.batch.guilds)
              this.clearGuilds(
                _this.settings.batch.muted
                  ? this.getGuilds()
                  : this.getUnread(),
              );
            if (_this.settings.batch.dms)
              BDFDB.DMUtils.markAsRead(this.getPingedDMs());
          }
          clearGuilds(guildIds) {
            BDFDB.GuildUtils.markAsRead(
              guildIds.filter((id) => id && !blacklist.includes(id)),
            );
          }
          getGuilds() {
            return BDFDB.LibraryStores.SortedGuildStore.getFlattenedGuildIds()
              .map(BDFDB.LibraryStores.GuildStore.getGuild)
              .map((g) => g.id)
              .filter((n) => n);
          }
          getUnread() {
            return this.getGuilds().filter(
              (id) =>
                BDFDB.LibraryStores.GuildReadStateStore.hasUnread(id) ||
                BDFDB.LibraryStores.GuildReadStateStore.getMentionCount(id) > 0,
            );
          }
          getPinged() {
            return this.getGuilds().filter(
          ...
          getMuted() {
            return this.getGuilds().filter((id) =>
              BDFDB.LibraryStores.UserGuildSettingsStore.isGuildOrCategoryOrChannelMuted(
                id,
              ),
            );
          }
          getPingedDMs() {
            return BDFDB.LibraryStores.ChannelStore.getSortedPrivateChannels()
              .map((c) => c.id)
              .filter(
                (id) =>
                  id &&
                  BDFDB.LibraryStores.ReadStateStore.getMentionCount(id) > 0,
              );
          }
          render() {
            return BDFDB.ReactUtils.createElement("div", {
              className:
                BDFDB.disCNS.guildouter +
                BDFDB.disCN._readallnotificationsbuttonframe,
              children: BDFDB.ReactUtils.createElement("div", {
                className:
                  BDFDB.disCNS.guildiconwrapper +
                  BDFDB.disCN._readallnotificationsbuttoninner,
                children: BDFDB.ReactUtils.createElement("div", {
                  className:
                    BDFDB.disCNS.guildiconchildwrapper +
                    BDFDB.disCNS.guildiconchildwrappernohoverbg +
                    BDFDB.disCN._readallnotificationsbuttonbutton,
                  children: "Read All",
                  onClick: (_) => {
                    if (!_this.settings.general.confirmClear) this.clearClick();
                    else
                      BDFDB.ModalUtils.confirm(
                        _this,
                        _this.labels.modal_confirmnotifications,
                        (_) => this.clearClick(),
                      );
                  },
                  onContextMenu: (event) =>
                          ...
                          children: [
                            BDFDB.ContextMenuUtils.createItem(
                              BDFDB.LibraryComponents.MenuItems.MenuItem,
                              {
                                ...
                                action: (_) =>
                                  this.clearGuilds(this.getUnread()),
                              },
                            ),
                            BDFDB.ContextMenuUtils.createItem(
                              BDFDB.LibraryComponents.MenuItems.MenuItem,
                              {
                                ...
                                action: (_) =>
                                  this.clearGuilds(this.getPinged()),
                              },
                            ),
                            BDFDB.ContextMenuUtils.createItem(
                              BDFDB.LibraryComponents.MenuItems.MenuItem,
                              {
                                ...
                                action: (_) =>
                                  this.clearGuilds(this.getMuted()),
                              },
                            ),
                            BDFDB.ContextMenuUtils.createItem(
                              BDFDB.LibraryComponents.MenuItems.MenuItem,
                              {
                                ...
                                action: (_) =>
                                  this.clearGuilds(this.getGuilds()),
                              },
                            ),
                            BDFDB.ContextMenuUtils.createItem(
                              BDFDB.LibraryComponents.MenuItems.MenuItem,
        ...
        return class ReadAllNotificationsButton extends Plugin {
            ...
            this.defaults = {
              general: {
                addClearButton: {
                  value: true,
                  description:
                    "Adds a 'Clear Mentions' button to the recent mentions popout",
                },
                confirmClear: {
                  value: false,
                  description:
                    "Asks for your confirmation before clearing reads",
                },
              },
              batch: {
          ...
          onStart() {
            ...
            this.saveBlacklist(
              !BDFDB.ArrayUtils.is(loadedBlacklist) ? [] : loadedBlacklist,
            );
          ...
          getSettingsPanel(collapseStates = {}) {
                  ...
                  children: Object.keys(this.defaults.general)
                    ...
                    .concat(
                      BDFDB.ReactUtils.createElement(
                        ...
                        {
                          title:
                            "When left clicking the 'read all' Button mark following Elements as read:",
                          first: false,
                          last: true,
                          children: Object.keys(this.defaults.batch).map(
                            (key) =>
                              BDFDB.ReactUtils.createElement(
                                BDFDB.LibraryComponents.SettingsSaveItem,
                                {
            ...
            let listInstance = null,
              batchSetGuilds = (value) => {
                if (!value) {
                  for (let id of BDFDB.LibraryStores.SortedGuildStore.getFlattenedGuildIds())
                    blacklist.push(id);
            ...
            settingsItems.push(
              BDFDB.ReactUtils.createElement(
                BDFDB.LibraryComponents.CollapseContainer,
                {
                  ...
                  children: [
                    BDFDB.ReactUtils.createElement(
                      BDFDB.LibraryComponents.SettingsGuildList,
                      {
          ...
          processUnreadDMs(e) {
            ...
            e.returnvalue.push(
              BDFDB.ReactUtils.createElement(ReadAllButtonComponent, {}),
            );
          }

          processInboxHeader(e) {
            if (
              !this.settings.general.addClearButton ||
              e.instance.props.tab != BDFDB.DiscordConstants.InboxTabs.MENTIONS
            )
              return;
            let mentionedMessages =
              BDFDB.LibraryStores.RecentMentionsStore.getMentions();
            if (!mentionedMessages || !mentionedMessages.length) return;
            let controls = BDFDB.ReactUtils.findChild(e.returnvalue, {
              props: [["className", BDFDB.disCN.messagespopoutcontrols]],
            });
            if (controls)
                    ...
                    children: BDFDB.ReactUtils.createElement(
                      BDFDB.LibraryComponents.Clickable,
                      {
                        className:
                          ...
                          BDFDB.disCNS.messagespopoutbuttonsm +
                          BDFDB.disCN.messagespopoutbuttonsecondary,
                        children: BDFDB.ReactUtils.createElement("div", {
                          className:
                            BDFDB.disCN.messagespopoutbuttonchildrenwrapper,
                          children: BDFDB.ReactUtils.createElement("div", {
                        ...
                        onClick: (_) => {
                          let clear = (_) => {
                            ...
                            let messages = []
                              .concat(mentionedMessages)
                              .filter((n) => n);
                            if (messages.length) {
                              clearing = true;
                              let toast = BDFDB.NotificationUtils.toast(
                                `${this.labels.toast_clearing} - ${BDFDB.LanguageUtils.LibraryStrings.please_wait}`,
                                { timeout: 0, ellipsis: true },
                              );
                              for (let i = 0; i < messages.length; i++)
                                BDFDB.TimeUtils.timeout((_) => {
                                  BDFDB.LibraryModules.HTTPUtils.del({
                                    url: BDFDB.DiscordConstants.Endpoints.MENTIONS_MESSAGE_ID(
                                      messages[i].id,
                                    ),
                                    retries: 2,
                                    oldFormErrors: true,
                          ...
                          if (this.settings.general.confirmClear)
                            BDFDB.ModalUtils.confirm(
                              this,
                              this.labels.modal_confirmmentions,
                              clear,
                            );
                          else clear();
                        },
            ...
            switch (BDFDB.LanguageUtils.getLanguage().id) {
              case "bg": // Bulgarian
                return {
                  ...
                  modal_confirmmentions:
                    "Наистина ли искате да изтриете всички непрочетени споменавания?",
                  modal_confirmnotifications:
                    "Наистина ли искате да изтриете всички непрочетени известия?",
              ...
              case "da": // Danish
                return {
                  ...
                  modal_confirmmentions:
                    "Er du sikker på, at du vil slette alle ulæste omtaler?",
                  modal_confirmnotifications:
                    "Er du sikker på, at du vil slette alle ulæste meddelelser?",
              ...
              case "de": // German
                return {
                  ...
                  modal_confirmmentions:
                    "Möchten Sie wirklich alle ungelesenen Erwähnungen löschen?",
                  modal_confirmnotifications:
                    "Möchten Sie wirklich alle ungelesenen Benachrichtigungen löschen?",
              ...
              case "el": // Greek
                return {
                  ...
                  modal_confirmmentions:
                    "Είστε βέβαιοι ότι θέλετε να διαγράψετε όλες τις μη αναγνωσμένες αναφορές;",
                  modal_confirmnotifications:
                    "Είστε βέβαιοι ότι θέλετε να διαγράψετε όλες τις μη αναγνωσμένες ειδοποιήσεις;",
              ...
              case "es": // Spanish
                return {
                  ...
                  modal_confirmmentions:
                    "¿Estás seguro de que deseas eliminar todas las menciones no leídas?",
                  modal_confirmnotifications:
                    "¿Está seguro de que desea eliminar todas las notificaciones no leídas?",
                  toast_alreadyclearing: "Elimina algunas menciones ya",
                  toast_cleared:
                    "Se han eliminado todas las menciones recientes",
              ...
              case "fi": // Finnish
                return {
                  ...
                  modal_confirmmentions:
                    "Haluatko varmasti poistaa kaikki lukemattomat maininnat?",
                  modal_confirmnotifications:
                    "Haluatko varmasti poistaa kaikki lukemattomat ilmoitukset?",
              ...
              case "fr": // French
                return {
                  ...
                  modal_confirmmentions:
                    "Voulez-vous vraiment supprimer toutes les mentions non lues?",
                  modal_confirmnotifications:
                    "Voulez-vous vraiment supprimer toutes les notifications non lues?",
                  toast_alreadyclearing: "Supprime déjà certaines mentions",
                  toast_cleared:
                    "Toutes les mentions récentes ont été supprimées",
              ...
              case "hr": // Croatian
                return {
                  ...
                  modal_confirmmentions:
                    "Jeste li sigurni da želite izbrisati sva nepročitana spominjanja?",
                  modal_confirmnotifications:
                    "Jeste li sigurni da želite izbrisati sve nepročitane obavijesti?",
              ...
              case "hu": // Hungarian
                return {
                  ...
                  modal_confirmmentions:
                    "Biztosan törli az összes olvasatlan említést?",
                  modal_confirmnotifications:
                    "Biztosan törli az összes olvasatlan értesítést?",
              ...
              case "it": // Italian
                return {
                  ...
                  modal_confirmmentions:
                    "Sei sicuro di voler eliminare tutte le menzioni non lette?",
                  modal_confirmnotifications:
                    "Sei sicuro di voler eliminare tutte le notifiche non lette?",
                  toast_alreadyclearing: "Elimina già alcune menzioni",
                  toast_cleared:
                    "Tutte le menzioni recenti sono state eliminate",
              ...
              case "ja": // Japanese
                return {
                  ...
                  modal_confirmmentions:
                    "未読のメンションをすべて削除してもよろしいですか？",
                  modal_confirmnotifications:
                    "未読の通知をすべて削除してもよろしいですか？",
              ...
              case "ko": // Korean
                return {
                  ...
                  modal_confirmmentions:
                    "읽지 않은 모든 멘션을 삭제 하시겠습니까?",
                  modal_confirmnotifications:
                    "읽지 않은 모든 알림을 삭제 하시겠습니까?",
              ...
              case "lt": // Lithuanian
                return {
                  ...
                  modal_confirmmentions:
                    "Ar tikrai norite ištrinti visus neperskaitytus paminėjimus?",
                  modal_confirmnotifications:
                    "Ar tikrai norite ištrinti visus neperskaitytus pranešimus?",
              ...
              case "nl": // Dutch
                return {
                  ...
                  modal_confirmmentions:
                    "Weet u zeker dat u alle ongelezen vermeldingen wilt verwijderen?",
                  modal_confirmnotifications:
                    "Weet u zeker dat u alle ongelezen meldingen wilt verwijderen?",
              ...
              case "no": // Norwegian
                return {
                  ...
                  modal_confirmmentions:
                    "Er du sikker på at du vil slette alle uleste omtaler?",
                  modal_confirmnotifications:
                    "Er du sikker på at du vil slette alle uleste varsler?",
              ...
              case "pl": // Polish
                return {
                  ...
                  modal_confirmmentions:
                    "Czy na pewno chcesz usunąć wszystkie nieprzeczytane wzmianki?",
                  modal_confirmnotifications:
                    "Czy na pewno chcesz usunąć wszystkie nieprzeczytane powiadomienia?",
              ...
              case "pt-BR": // Portuguese (Brazil)
                return {
                  ...
                  modal_confirmmentions:
                    "Tem certeza de que deseja excluir todas as menções não lidas?",
                  modal_confirmnotifications:
                    "Tem certeza de que deseja excluir todas as notificações não lidas?",
              ...
              case "ro": // Romanian
                return {
                  ...
                  modal_confirmmentions:
                    "Sigur doriți să ștergeți toate mențiunile necitite?",
                  modal_confirmnotifications:
                    "Sigur doriți să ștergeți toate notificările necitite?",
              ...
              case "ru": // Russian
                return {
                  ...
                  modal_confirmmentions:
                    "Вы уверены, что хотите удалить все непрочитанные упоминания?",
                  modal_confirmnotifications:
                    "Вы действительно хотите удалить все непрочитанные уведомления?",
              ...
              case "sv": // Swedish
                return {
                  ...
                  modal_confirmmentions:
                    "Är du säker på att du vill ta bort alla olästa omnämnanden?",
                  modal_confirmnotifications:
                    "Är du säker på att du vill ta bort alla olästa aviseringar?",
              ...
              case "th": // Thai
                return {
                  ...
                  modal_confirmnotifications:
                    "แน่ใจไหมว่าต้องการลบการแจ้งเตือนที่ยังไม่ได้อ่านทั้งหมด",
              ...
              case "tr": // Turkish
                return {
                  ...
                  modal_confirmmentions:
                    "Okunmamış tüm bahisleri silmek istediğinizden emin misiniz?",
                  modal_confirmnotifications:
                    "Okunmamış tüm bildirimleri silmek istediğinizden emin misiniz?",
              ...
              case "uk": // Ukrainian
                return {
                  ...
                  modal_confirmmentions:
                    "Ви впевнені, що хочете видалити всі непрочитані згадки?",
                  modal_confirmnotifications:
                    "Ви впевнені, що хочете видалити всі непрочитані сповіщення?",
              ...
              case "vi": // Vietnamese
                return {
                  ...
                  modal_confirmmentions:
                    "Bạn có chắc chắn muốn xóa tất cả các đề cập chưa đọc không?",
                  modal_confirmnotifications:
                    "Bạn có chắc chắn muốn xóa tất cả các thông báo chưa đọc không?",
              ...
              default: // English
                return {
                  ...
                  modal_confirmmentions:
                    "Are you sure you want to delete all unread Mentions?",
                  modal_confirmnotifications:
