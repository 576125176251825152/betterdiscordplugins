...
const checkObjectValues = (target) =>
  target !== window &&
...
const find = (filter, { resolve = true, entries = false } = {}) =>
...
const demangle = (mapping, required, proxy = false) => {
  ...
  const found = find(
    (target) =>
      checkObjectValues(target) &&
      req.every((req) =>
        Object.values(target).some((value) => mapping[req](value)),
      ),
  );
  return proxy
    ? mappedProxy(
...
const waitFor = (filter, { resolve = true, entries = false } = {}) =>
...
const after = (object, method, callback, options = {}) =>
  patch(
    "after",
    object,
    method,
    (cancel, original, context, args, result) =>
      callback({ cancel, original, context, args, result }),
    options,
  );
let menuPatches = [];
const unpatchAll = () => {
  if (
    menuPatches.length +
      BdApi.Patcher.getPatchesByCaller(getMeta().name).length >
    0
  ) {
...
const Flex = /* @__PURE__ */ byKeys(["Child", "Justify", "Align"], {
  entries: true,
});

const FormDivider = /* @__PURE__ */ bySource(
...
const SettingsContainer = ({ name, children, onReset }) =>
  React.createElement(
    "div",
    null,
    children,
...
class SettingsStore {
  ...
  update = (settings) => {
    const update =
      typeof settings === "function" ? settings(this.current) : settings;
...
const createSettings = (defaults, onLoad) =>
  new SettingsStore(defaults, onLoad);

const createPlugin = (plugin) => (meta) => {
  setMeta(meta);
  const { start, stop, styles, Settings, SettingsPanel } =
...
const hasOverride = (userId, context) =>
...
const dispatchVolumeOverrides = () => {
  ...
  for (const [userId, contexts] of Object.entries(
    Settings.current.volumeOverrides,
  )) {
