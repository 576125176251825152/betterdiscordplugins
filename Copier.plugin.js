...
const manifest = {
  ...
  description: "Allows you to copy certain stuff with custom options.",
  source:
    "https://github.com/Strencher/BetterDiscordStuff/blob/master/Copier/Copier.plugin.js",
  invite: "gvA2ree",
  changelog: [
    {
      title: "Bug Fixes",
      type: "fixed",
      items: ["Fixed role context menu", "Fixed message context menu"],
    },
  ],
  changelogDate: "2026-01-26",
};
...
function showChangelog(manifest) {
  ...
  const title = React.createElement(
    ...
    React.createElement("h1", null, "What's New - ", manifest.name),
    React.createElement(
      "div",
      null,
      formatter.format(new Date(manifest.changelogDate)),
      " - v",
      manifest.version,
    ),
  );
  const items = manifest.changelog.map((item) =>
    React.createElement(
      ...
      item.items.map((item2) => React.createElement("span", null, item2)),
    ),
  );
  "changelogImage" in manifest &&
    items.unshift(
...
const Settings$1 = {
  ...
  _settings: Object.assign(
    {},
    {
      ...
      voiceCustom:
        "**#$name** in **$server** with `$usersConnected` connected users.",
...
const MessageCopyOptions = [
  ...
  },
  {
    ...
    description:
      "Will be replace with the message author's tag. (Username#1234)",
  },
  {
    name: "authorMention",
    getValue: ({ message }) => `<@!${message.author.id}>`,
    description:
      "Will be replaced with the mention of the message author. (<@!userId>)",
  },
  {
    name: "author",
  ...
  },
  {
    ...
    description:
      "Will be replaced with the channel id where the message was sent.",
  },
  {
    name: "channelMention",
    getValue: ({ message }) => `<#${message.channel_id}>`,
    description:
      "Will be replaced with the mention of the channel where the message was sent.",
  },
];
...
function message() {
  return ContextMenu.patch("message", (res, props) => {
    ...
    const menuGroup = (
      findGroupById(res, "delete") || findGroupById(res, "report")
    )?.props?.children;
    const buttonIndex = menuGroup?.findIndex(
      (i) => i?.props?.id === "delete" || i?.props?.id === "report",
    );
          ...
          items: [
            ...
            },
            {
              ...
              action: () => {
                ...
                copy$1(
                  getMessageLink(channel.guild_id, channel.id, message.id),
                );
...
const className =
  Webpack$1.getByProps("dangerous", "button")?.button ?? "buttonUndefined";

function CopyButton(props) {
  const shouldShow = useStateFromStores([Settings$1], () =>
    Settings$1.get("showButton", true),
  );
...
Styles.sheets.push(
  "/* components/settings/textbox.scss */",
  `.copier-textbox .copier-textbox-note {
  font-size: 14px;
  color: var(--text-muted);
}
...
const VoiceChannelCopyOptions = [
  ...
  },
  {
    name: "usersConnected",
    getValue: ({ channel }) =>
      Object.keys(SortedVoiceStateStore.getVoiceStatesForChannel(channel))
        .length,
    description:
      "Will be replaced with the amount of users that are connected to the voice channel.",
  },
  {
    name: "channelMention",
    getValue: ({ channel }) => `<#${channel.id}>`,
  ...
  },
  {
    ...
    getValue: ({ channel }) => {
      const channels = Object.values(
        GuildChannelStore.getSelectableChannels(channel.guild_id),
      ).filter((child) => child.parent_id === channel.id);
      return channels.length;
    },
    description:
      "Will be replaced with the children channels count of that category.",
  },
];
...
function dev() {
  ...
  const patch = (res, props) => {
    ...
    res.props.children = res.props.children.filter(
      (e) => e && e?.props?.id !== "devmode-copy-id",
    );
          ...
          items: [
            ...
            },
            {
              ...
              items: [
                {
                  label: () =>
                    React.createElement(RoleColoredLabel, {
                      color: colors.hex,
                      label: "RGB",
                    }),
...
const Dynamic = ({ component: Type, ...props }) =>
...
class Copier {
  ...
  start() {
    ...
    for (const id in ContextMenus) {
      try {
        ...
        console.error(
          "[Copier] Could not initialize patch for",
          id,
          ":",
          error,
        );
  ...
  patchAboutMe() {
    const module = Webpack$1.getBySource([
      "animUserProfileSidebarateOnHoverOrFocusOnly",
      "61W33d",
    ]);
    const CopyButton2 = React.memo(({ onClick }) =>
      React.createElement(
        Tooltip,
        {
          text: "Copy About Me",
    ...
    Patcher.after(module, "Z", (_, [{ bio }], res) => {
      ...
      const title = findInTree(
        res,
        (e) => e?.variant && Array.isArray(e.children),
      );
      if (!title) return res;
      title.children.push(
    ...
    const buttonsContainer = await new Promise((resolve) => {
      onceAdded(
        ...
        (node) => {
          ...
          if (!instance) return;
          for (
            let curr = instance, max = 100;
            curr != null && max--;
            curr = curr?.return
          ) {
            max < 5 && console.log(curr);
            if (
              (node = curr?.memoizedProps?.children?.type) &&
              node?.$$typeof
            ) {
              resolve(node);
              break;
