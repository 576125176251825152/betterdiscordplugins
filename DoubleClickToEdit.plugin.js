...
module.exports = class DoubleClickToEdit {
  ...
  start() {
    try {
      //Classes
      this.selectedClass = Webpack.getModule(
        Filters.byKeys("message", "selected"),
      ).selected;
      ...
      //Copy to clipboard
      this.copyToClipboard = Webpack.getModule(
        Filters.byKeys("clipboard", "app"),
      ).clipboard.copy;
      ...
      //Stores
      this.MessageStore = Webpack.getModule(
        Filters.byKeys("receiveMessage", "editMessage"),
      );
      this.CurrentUserStore = Webpack.getModule(
        Filters.byKeys("getCurrentUser"),
      );

      //Settings
      this.FormSwitch = Webpack.getModule(
        Filters.byStrings("labelRow", "checked"),
        { searchExports: true },
      );
      this.RadioGroup = Webpack.getModule(
        (m) => Filters.byKeys("NOT_SET", "NONE")(m?.Sizes),
        { searchExports: true },
      );
      this.FormItem = Webpack.getModule(
        (m) =>
          Filters.byStrings("titleId", "errorId", "setIsFocused")(m?.render),
        { searchExports: true },
      );
      ...
      this.editModifier =
        Data.load(config.info.slug, "editModifier") ?? "shift";
      //Reply
      this.doubleClickToReply =
        Data.load(config.info.slug, "doubleClickToReply") ?? false;
      ...
      //Copy
      this.doubleClickToCopy =
        Data.load(config.info.slug, "doubleClickToCopy") ?? false;
      this.copyModifier =
        Data.load(config.info.slug, "copyModifier") ?? "shift";
    } catch (err) {
      try {
  ...
  handler(e) {
    ...
    if (
      ...
      ignore.some(
        (nameOfClass) => e?.target?.className?.indexOf?.(nameOfClass) > -1,
      )
    )
      return;
    ...
    const copyKeyHeld = this.checkForModifier(
      this.doubleClickToCopy,
      this.copyModifier,
      e,
    );
    ...
    const message =
      Utils.findInTree(instance, (m) => m?.baseMessage, { walkable: walkable })
        ?.baseMessage ??
      Utils.findInTree(instance, (m) => m?.message, { walkable: walkable })
        ?.message;

    if (!message) return;

    //Now we do the same thing with the edit and reply modifier
    const editKeyHeld = this.checkForModifier(
        this.doubleClickToEditModifier,
        this.editModifier,
        e,
      ),
      replyKeyHeld = this.checkForModifier(
        this.doubleClickToReplyModifier,
        this.replyModifier,
        e,
      );
    ...
    if (
      ...
      this.MessageStore.startEditMessage(
        message.channel_id,
        message.id,
        message.content,
      );
    else if (
      (this.doubleClickToReplyModifier ? replyKeyHeld : true) &&
      this.doubleClickToReply
    )
