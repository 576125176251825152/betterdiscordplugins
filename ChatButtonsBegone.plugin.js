...
class Styler {
  ...
  apply() {
    this.api.DOM.addStyle(
      this.pluginName,
      `${this.styles.join(", ")} { display: none !important; }`,
    );
  }
...
module.exports = class ChatButtonsBegone {
  constructor(meta) {
    ...
    [
      ...
      {
        filter: this.api.Webpack.Filters.byKeys(
          "messageListItem",
          "message",
          "buttons",
        ),
      ...
      }, // Server Unread Mentions Indicator: Bottom
      ...
      { filter: this.api.Webpack.Filters.byKeys("clanTagChiplet") }, // Clan Tag Chiplet
      {
        filter: this.api.Webpack.Filters.byKeys(
          ...
          "chipletContainerInline",
        ),
      }, // Clan Tag Chiplet in Server
      { filter: this.api.Webpack.Filters.byKeys("avatarDecorationContainer") }, // Avatar Decoration
      {
        filter: this.api.Webpack.Filters.byKeys("avatarDecoration", "contents"),
      }, // Avatar Decoration in Chat
    ...
    // Add Reaction Button
    this.addReactionButton = BdApi.Webpack.getAllByKeys(
      "reactionBtn",
      "forceShow",
      "reactionMe",
      "reactionReadOnly",
  ...
  migrateConfig() {
    const migrations = [
      ...
      },
      {
        to: "3.2.0",
        migrate: (config) => {
          config.servers.channelsAndRoles =
            config.servers.channelsAndRoles || config.servers.browseChannels;
    ...
    for (const category of config.defaultConfig) {
      if (category.type === "category") {
        ...
        for (const setting of category.settings) {
          if (
            !(setting.id in this.settings[category.id]) ||
            this.settings[category.id][setting.id] == null
          ) {
  ...
  addStyles() {
    /// Chat Buttons ///
    if (this.settings.chatbar.attachButton)
      this.styler.add(`.${this.attachButton.attachWrapper}`);
    if (this.settings.chatbar.giftButton) {
      // New Implementation (Valentines Upsell)
      this.styler.add(
        `.${this.chatBarButtons.buttons} div[class^="container"]:has(>.${this.chatBarButtons.button})`,
      );
      // Old Implementation
      this.styler.add(
        `.${this.chatBarButtons.buttons} > .${this.chatBarButtons.button}:not(.expression-picker-chat-input-button)`,
      );
    }
    if (this.settings.chatbar.gifButton)
      this.styler.add(
        `.expression-picker-chat-input-button:not(:has(.${this.chatBarButtons.stickerButton}, .${this.chatBarButtons.emojiButton?.split(" ")[0]}))`,
      );
    if (this.settings.chatbar.stickerButton)
      this.styler.add(
        `.expression-picker-chat-input-button:has(.${this.chatBarButtons.stickerButton})`,
      );
    if (this.settings.chatbar.emojiButton)
      this.styler.add(
        `.expression-picker-chat-input-button:has(.${this.chatBarButtons.emojiButton?.split(" ")[0]})`,
      );
    if (this.settings.chatbar.appLauncherButton)
      this.styler.add(".app-launcher-entrypoint");
    ...
    if (
      ...
      this.styler.add(
        `.${this.messageActionContainer.message} .${this.messageActionContainer.buttons}`,
      );
    }
    if (this.settings.messageActions.quickReactions) {
      this.styler.add(
        `.${this.messageActionButtons.hoverBarButton}:has(>.${this.messageActionButtons.icon}>[data-type="emoji"])`,
      );
      this.styler.add(`.${this.messageActionButtons.separator}`);
    }
    if (this.settings.messageActions.reactionButton)
      this.styler.add(
        `.${this.messageActionButtons.hoverBarButton}:has(svg>path[d^="M12 23a11 11 0 1 0 0-22 11 11 0 0 0 0 22ZM6.5"])`,
      );
    if (this.settings.messageActions.editButton)
      this.styler.add(
        `.${this.messageActionButtons.hoverBarButton}:has(svg>path[d^="m13.96 5.46 4.58 4.58a1 1 0 0 0 1.42 0l1.38-1.38a2"])`,
      );
    if (this.settings.messageActions.replyButton)
      this.styler.add(
        `.${this.messageActionButtons.hoverBarButton}:has(svg>path[d^="M2.3 7.3a1 1 0 0 0 0 1.4l5 5a1 1 0 0 0 1.4-1.4L5.42"])`,
      );
    if (this.settings.messageActions.forwardButton)
      this.styler.add(
        `.${this.messageActionButtons.hoverBarButton}:has(svg>path[d^="M21.7 7.3a1 1 0 0 1 0 1.4l-5 5a1 1 0 0 1-1.4-1.4L18.58"])`,
      );
    if (this.settings.messageActions.removeMore)
      this.styler.add(
        `.${this.messageActionButtons.hoverBarButton}:has(svg>path[d^="M4 14a2 2 0 1 0 0-4 2 2 0 0 0 0 4Zm10-2a2"])`,
      );

    if (this.settings.messageActions.addReactionButton)
      this.styler.add(`.${this.addReactionButton.reactionBtn}`);

    /// Direct Messages ///
    if (this.settings.dms.quickSwitcher)
      this.styler.add(`.${this.DMList.privateChannels} [class^="searchBar"]`);
    if (this.settings.dms.friendsTab)
      this.styler.add('li:has([href="/channels/@me"])');
    if (this.settings.dms.premiumTab)
      this.styler.add('li:has([href="/store"])');
    ...
    if (this.settings.dms.DMHeader == "hideButton") {
      this.styler.add(
        `.${this.DMHeader.privateChannelRecipientsInviteButtonIconContainer}`,
      );
    ...
    if (this.settings.dms.activeNow == "simplify") {
      this.styler.add(
        `.${this.activeNowCards.body}:has(.${this.activeNowCards.twitchSectionPreview})`,
      );
      this.styler.add(
        `.${this.activeNowCards.body}:has(.${this.activeNowCards.activitySection})`,
      );
    } else if (this.settings.dms.activeNow == "empty") {
      this.styler.add(
        `.${this.activeNowColumn.nowPlayingColumn}:has(.${this.activeNowEmpty.emptyCard})`,
      );
    ...
    if (this.settings.dms.libraryTab)
      this.styler.add('li:has([href="/library"])');

    /// Servers and Channels ///
    if (this.settings.servers.addServerButton)
      this.styler.add(
        `.${this.addServerDiscoverButton.tutorialContainer}:not(:first-child)`,
      );
    if (this.settings.servers.discoverButton)
      this.styler.add(
        `.${this.addServerDiscoverButton.tutorialContainer} + .${this.addServerDiscoverButton.listItem}`,
      );

    if (this.settings.servers.unreadIndicator == "both") {
      this.styler.add(
        `.${this.indicatorTop.unreadMentionsIndicatorTop}, .${this.indicatorBottom.unreadMentionsIndicatorBottom}`,
      );
    ...
    if (this.settings.servers.serverBanner) {
      ...
      this.styler.add(
        'div[id="channels"] > ul :is(div[style="height: 84px;"], div[style="height: 8px;"], div[style="height: 12px;"])',
      );
    }
    if (this.settings.servers.boostBar)
      this.styler.add(`.${this.boostBar.container}`);
    if (this.settings.servers.serverGuide)
      this.styler.add('li:has(div[id*="home-tab"])');
    if (this.settings.servers.eventButton)
      this.styler.add(
        '[id="channels"] li:has(svg>path[d^="M7 1a1 1 0 0 1 1 1v.75c0 .14.11.25.25.25h7.5c.14 0"])',
      );
    if (this.settings.servers.membersButton)
      this.styler.add(
        '[id="channels"] li:has(svg>path[d^="M14.5 8a3 3 0 1 0-2.7-4.3c-.2.4.06.86.44 1.12a5"])',
      );
    if (this.settings.servers.channelsAndRoles)
      this.styler.add(
        '[id="channels"] li:has(svg>path[d^="M18.5 23c.88 0 1.7-.25 2.4-.69l1.4 1.4a1"])',
      );
    if (this.settings.servers.boostsButton)
      this.styler.add('li:has(div[id*="skill-trees"])');
    if (this.settings.servers.shopButton)
      this.styler.add('div:has(> li > div > [id^="game-shop"])');
    if (this.settings.servers.inviteButton) {
      this.styler.add(`.${this.headerInviteButton.inviteButton}`);
      this.styler.add(
        `.${this.channelListButtons.children}>span:has(svg>path[d^="M19 14a1 1 0 0 1 1 1v3h3a1 1 0 0 1"])`,
      );
    }
    if (this.settings.servers.settingsButton)
      this.styler.add(
        `.${this.channelListButtons.children}>span:has(svg>path[d^="M10.56 1.1c-.46.05-.7.53-.64.98.18 1.16-.19 2.2-.98"])`,
      );
    if (this.settings.servers.activitySection) {
      this.styler.add(
        `.${this.serverActivitySection.membersGroup}:has([role="button"])`,
      );
      this.styler.add(
        `div > div .${this.serverActivitySectionCards.usesCardRows}`,
      );
      this.styler.add(
        `div > div .${this.serverActivityOnHover.container}.${this.serverActivityOnHover.openOnHover}`,
      );
    }

    /// Voice ///
    if (this.settings.voice.invitePlaceholder)
      this.styler.add(
        `div[class^="row"]>div:has(.${this.vcScreen.singleUserRoot})`,
      );
    if (this.settings.voice.cameraPanelButton)
      this.styler.add(
        `.${this.vcButtons.actionButtons} > button:first-of-type`,
      );
    if (this.settings.voice.screensharePanelButton)
      this.styler.add(
        `.${this.vcButtons.actionButtons} > button:nth-of-type(2)`,
      );
    if (this.settings.voice.activityPanelButton)
      this.styler.add(
        `.${this.vcButtons.actionButtons} > button:nth-of-type(3)`,
      );
    if (this.settings.voice.soundboardPanelButton)
      this.styler.add(`.${this.vcButtons.actionButtons} span:has(svg)`);
    if (this.settings.voice.krispButton)
      this.styler.add(
        `.${this.vcKrisp.voiceButtonsContainer} button:first-of-type`,
      );
    if (this.settings.voice.gameActivityPanel)
      this.styler.add(`.${this.vcActivityPanel.activityPanel}`);
    if (this.settings.voice.gameActivityButton)
      this.styler.add(
        `.${this.vcButtonSection.buttonContainer}:has(.${this.vcActivities.attachedCaretButtonContainer})`,
      );
    if (this.settings.voice.voiceAvatars)
      this.styler.add(`.${this.scSmallAvatar.avatarSmall}`);

    /// Title Bar ///
    if (this.settings.toolbar.navButtons)
      this.styler.add(`.${this.backForwardButtons.backForwardButtons}`);
    if (this.settings.toolbar.locator)
      this.styler.add(`.${this.titleBarTrailing.title}`);
    if (this.settings.toolbar.checkpointButton)
      this.styler.add(
        `:is(.${this.titleBarTrailing.trailing}, .${this.upperToolbar.toolbar}) div:has(>svg>path[d^="M5.1 1a2.1 2.1 0 0 1 1.8 3.14h14.05c.84"])`,
      );
    if (this.settings.toolbar.helpButton)
      this.styler.add(
        `:is(.${this.titleBarTrailing.trailing}, .${this.upperToolbar.toolbar}) a[href="https://support.discord.com"]`,
      );
    if (this.settings.toolbar.inboxButton)
      this.styler.add(
        `:is(.${this.titleBarTrailing.trailing}, .${this.upperToolbar.toolbar}) div:has(svg>path[d^="M5 2a3 3 0 0 0-3 3v14a3 3 0 0 0 3 3h14a3"])`,
      );

    /// Toolbar ///
    if (this.settings.toolbar.threadsButton)
      this.styler.add(
        `.${this.upperToolbar.iconWrapper}:has(svg>path[d^="M12 2.81a1 1 0 0 1 0-1.41l.36-.36a1 1 0 0 1 1.41 0l9.2 9.2a1"]) `,
      );
    if (this.settings.toolbar.notifyButton)
      this.styler.add(
        `.${this.upperToolbar.iconWrapper}:has(>svg>path[d^="M1.3 21.3a1 1 0 1 0 1.4 1.4l20-20a1"]) `,
      );
    if (this.settings.toolbar.pinnedButton)
      this.styler.add(
        `.${this.upperToolbar.iconWrapper}:has(>svg path[d^="M19.38 11.38a3 3 0 0 0 4.24 0l.03-.03a.5.5 0 0 0 0-.7L13.35.35a.5.5"]) `,
      );
    if (this.settings.toolbar.memberButton)
      this.styler.add(
        `.${this.upperToolbar.iconWrapper}:has(>svg>path[d^="M14.5 8a3 3 0 1 0-2.7-4.3c-.2.4.06.86.44 1.12a5"]) `,
      );
    if (this.settings.toolbar.voiceButton)
      this.styler.add(
        `.${this.upperToolbar.iconWrapper}:has(svg>path[d="M13 7a1 1 0 0 1 1-1 4 4 0 0 1 4 4 1 1 0 1 1-2 0 2 2 0 0 0-2-2 1 1 0 0 1-1-1Z"]) `,
      );
    if (this.settings.toolbar.videoButton)
      this.styler.add(
        `.${this.upperToolbar.iconWrapper}:has(>svg>path[d^="M4 4a3 3 0 0 0-3 3v10a3"])`,
      );
    if (this.settings.toolbar.profileButton)
      this.styler.add(
        `.${this.upperToolbar.iconWrapper}:has(>svg>path[d^="M23 12.38c-.02.38-.45.58-.78.4a6.97 6.97 0 0 0-6.27-.08.54.54"]) `,
      );
    ...
    if (this.settings.profileCustomizations.namePlate == "original") {
      // Server List / DM List
      this.styler.add(
        `.${this.namePlate.nameplated} > [style*="linear-gradient"], .${this.dmEntry.interactive} > [style^="background: linear-gradient"]`,
      );
    ...
    if (this.settings.profileCustomizations.clanTag == "memberlist") {
      ...
      this.styler.add(
        `:not(.${this.clanTagProfile.guildTagContainer}, #guild-header-popout-guild-tag) > .${this.clanTagChipletServer.chipletContainerInner}`,
      );
      ...
      // Server List
      this.styler.add(
        `:not(#guild-header-popout-guild-tag) > .${this.clanTagChipletServer.chipletContainerInner}`,
      );
      // Profile
      this.styler.add(`.${this.clanTagProfile.guildTagContainer}`);
    }
    ...
    if (this.settings.profileCustomizations.hideBadges)
      this.styler.add(`.${this.profileBadges.tags} > div:has(a)`);
    if (this.settings.profileCustomizations.profileEffects)
      this.styler.add(
        `.${this.profileEffects.profileEffects} .${this.profileEffects.effect}`,
      );
    if (this.settings.profileCustomizations.profileGIF)
      this.styler.add(`.${this.profileGIF.gifTag}`);
    if (this.settings.profileCustomizations.hideCollection) {
      this.styler.add(
        `.${this.profileCollection.cardsList}:has([class^="breadcrumb"])`,
      );
      this.styler.add(`.${this.profileWidgets.widgetPreviews}`);
    }
    if (this.settings.profileCustomizations.hideWishlist)
      this.styler.add(`.${this.profileWishlist.wishlistBreadcrumb}`);
    if (this.settings.profileCustomizations.hideStatus)
      this.styler.add(`.${this.profileCustomStatus.ring}`);

    /// Miscellaneous ///
    if (this.settings.miscellaneous.blockedMessage)
      this.styler.add(
        `.${this.blockedGroup.groupStart}:has(.${this.blockedIndicator.blockedSystemMessage})`,
      );

    if (this.settings.miscellaneous.nitroUpsell) {
      // Settings "Edit Profile" Page
      this.styler.add(
        `.${this.shopArt.settingsPage} div:has(>[class^="artContainer"])`,
      );
      // Billing Settings
      this.styler.add(
        '[data-settings-sidebar-item="nitro_panel"], [data-settings-sidebar-item="premium_guild_subscriptions_panel"], [data-settings-sidebar-item="gift_panel"]',
      );
      ...
      // Profile Shop Button
      this.styler.add(
        `[class^="profile"] [class^="profileButtons"] > span:first-of-type`,
      );
    }
    ...
    if (this.settings.miscellaneous.placeholderText)
      this.styler.add(
        `.${this.txtPlaceholder.slateTextArea}:has(+ .${this.txtPlaceholder.slateTextArea})`,
      );
    if (this.settings.miscellaneous.avatarPopover)
      this.styler.add(`.${this.profilePopover.statusPopover}`);
    ...
    if (this.settings.miscellaneous.seasonalEvents)
      this.styler.add(
        '[href="//discord.com/snowsgiving"], [href="/activities"]',
      );
    if (this.settings.miscellaneous.ioChevrons)
      this.styler.add(`.${this.iochevron.buttonChevron}`);
    if (this.settings.miscellaneous.baseGradient)
      this.styler.add(`.${this.typeGradient.chatGradientBase}`);
    if (this.settings.miscellaneous.tagsBotApp)
      this.styler.add(
        `.${this.tagsBot.botTag}:not(.${this.tagsBot.botTagOP?.split(" ")[0]})`,
      );
    if (this.settings.miscellaneous.badgeNewUser)
      this.styler.add(`.${this.badgeNew.newMemberBadge}`);

    // Remove Custom User Status
    if (this.settings.miscellaneous.userStatus == "dmlist") {
      this.styler.add(
        `.${this.dmStatus.textXs}:has(.${this.dmlistStatus.activityStatusText})`,
      );
    ...
    /// Compatibility ///
    if (this.settings.compatibility.invisibleTypingButton)
      this.styler.add(`div:has(>.invisibleTypingButton)`);
  ...
  async start() {
    ...
    try {
      ...
      this.api.Logger.error(
        `Failed to apply styles. Please report the following error to ${config.info.github}/issues:\n\n${error}`,
      );
      BdApi.UI.showToast(
        "ChatButtonsBegone encountered an error! Check the console for more information.",
