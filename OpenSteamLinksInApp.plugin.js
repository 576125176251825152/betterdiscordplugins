    ...
    : (([Plugin, BDFDB]) => {
        ...
        return class OpenSteamLinksInApp extends Plugin {
                    ...
                    .map((url) =>
                      url.indexOf("http") == 0
                        ? url.indexOf("https://") == 0
                          ? [
                              `a[href^="${url}"]`,
                              `a[href^="${url.replace(/https:\/\//i, "http://")}"]`,
                            ]
                          : `a[href^="${url}"]`
                        : `a[href*="${url}"][href*="${key}"]`,
                    )
                ...
                (e) => {
                  if (
                    !(
                      ...
                      e.currentTarget.className.indexOf(BDFDB.disCN.imagezoom) >
                        -1
                    ) &&
                    !BDFDB.DOMUtils.getParent(
                      BDFDB.dotCN.imagezoom,
                      e.currentTarget,
                    )
                  )
          ...
          processMessageContent(e) {
            ...
            for (let i in e.instance.props.content)
              if (
                e.instance.props.content[i] &&
                e.instance.props.content[i].type == "span" &&
                typeof e.instance.props.content[i].props.children == "string" &&
                e.instance.props.content[i].props.children.indexOf(
                  "steam://",
                ) == 0
              ) {
                const url = e.instance.props.content[i].props.children;
                e.instance.props.content[i] = BDFDB.ReactUtils.createElement(
                  BDFDB.LibraryComponents.Anchor,
                  {
                    href: url,
                    title: url,
                    onClick: (_) =>
                      BDFDB.LibraryRequires.electron.shell.openExternal(url),
          ...
          openIn(e, key, url) {
            ...
            if (
              url &&
              !url.startsWith("https://images-ext-1.discord") &&
              !url.startsWith("https://images-ext-2.discord") &&
              typeof this[`openIn${platform}`] == "function"
            ) {
          ...
          async openInSteam(url) {
            ...
            if (
              BDFDB.LibraryRequires.electron.shell.openExternal(
                "steam://openurl/" + responseUrl,
              )
            );
