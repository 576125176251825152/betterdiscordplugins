...
class SimleImageViewer {
  ...
  constructor(image, imageWrapper, backdrop) {
    ...
    this.initialTransition &&
      this.transitionStyles.push(this.initialTransition);
    ...
    this.imageObserver.observe(this.imageWrapper, {
      childList: true,
      subtree: true,
    });
  }
  ...
  onWheel = (ev) => {
    ...
    if (targetScale <= 1) {
      ...
      const left =
        (this.wrapperWidth - width) / 2 + this.deltaX + innerWidth * 0.05;
      const right =
        (this.wrapperWidth + width) / 2 + this.deltaX + innerWidth * 0.05;
      const top =
        (this.wrapperHeight - height) / 2 + this.deltaY + innerHeight * 0.05;
      const bottom =
        (this.wrapperHeight + height) / 2 + this.deltaY + innerHeight * 0.05;
      ...
      const dx = isCursorOutSide
        ? 0
        : ((centerX - clamp(ev.clientX, left, right)) / this.scale) *
          deltaScale;
      const dy = isCursorOutSide
        ? 0
        : ((centerY - clamp(ev.clientY, top, bottom)) / this.scale) *
          deltaScale;
      if (isZoomIn) {
        this.deltaX += dx;
        this.deltaY += dy;
  ...
  move(dx, dy) {
    ...
    const deltaWidth = Math.max(
      (width * this.scale - this.wrapperWidth) / 2,
      0,
    );
    const deltaHeight = Math.max(
      (height * this.scale - this.wrapperHeight) / 2,
      0,
    );
...
function getImageWrapperFromAddedNode(element) {
  ...
  if (element.matches('div[class*="layer_"]')) {
    return /** @type {HTMLDivElement | null} */ (
      element.querySelector(IMAGE_WRAPPER_SELECTOR)
    );
  }
  // Gallery view
