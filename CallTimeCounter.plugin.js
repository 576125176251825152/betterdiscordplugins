...
const PanelSubtext = Webpack.getModule(
  (m) =>
    m?.$$typeof?.toString() === "Symbol(react.forward_ref)" &&
...
class Timer extends Component {
    ...
    if (lastVoice === SelectedChannelStore.getVoiceChannelId()) {
      ...
      this.interval = setInterval(() => {
        this.setState(
          (prev) =>
            (prev.delta =
              Math.round((Date.now() - prev.startTime) / 1000) * 1000),
        );
        this.setState(
          (prev) => (prev.lastVoice = SelectedChannelStore.getVoiceChannelId()),
        );
      }, 1000);
    } else {
      this.setState((prev) => (prev.startTime = Date.now()));
  ...
  render() {
    return createElement(
      "div",
      { className: "voiceTimer" },
      `Time elapsed: ${new Date(this.state.delta).toISOString().substring(11, 19)}`,
    );
  }
}
