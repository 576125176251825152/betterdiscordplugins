...
const { Patcher, Webpack, DOM, React, Data, UI } = new BdApi(
  "BetterMediaPlayer",
);
const classes = Object.assign(
  {},
  Webpack.getModule((m) => m.controlIcon && m.video),
  Webpack.getModule((m) => m.button && m.colorBrand),
);
...
const [
  ...
  {
    filter: Webpack.Filters.byStrings('"useStateFromStores"'),
    searchExports: true,
  },
  {
    filter: (m) =>
      m.render?.toString().includes("Missing guestWindow reference"),
  },
  ...
  {
    filter: (m) =>
      typeof m.highlight === "function" && typeof m.highlightAll === "function",
  },
  ...
  { filter: Webpack.Filters.byKeys("guildIcon", "title", "button") },
  {
    filter: (
      (m) => (exports, module) =>
        m(Webpack.modules[module.id])
    )(Webpack.Filters.byStrings("data-text-variant")),
    searchExports: true,
  },
  {
    filter: (m) => typeof m === "function" && typeof m.Link === "function",
    searchExports: true,
  },
);
...
const [toolbarModule, toolbarKey] = Webpack.getWithKey(
  Webpack.Filters.byStrings(".PlatformTypes.WINDOWS", "leading:"),
  {
...
function Duration({ currentTime, duration }) {
  const parsedCurrentTime = React.useMemo(
...
class ErrorBoundary extends React.Component {
  state = {
    didError: false,
    errorInfo: new Error(
      "Minified React error #999; Fake React Error (for debugging)",
    ),
  };
  ...
  render() {
    return React.createElement(React.Fragment, {
      children: [
        this.state.didError &&
          React.createElement(ErrorSplash, {
            ...this.props,
            errorInfo: this.state.errorInfo,
          }),
        !this.state.didError &&
          React.createElement(PictureInPicture, this.props),
      ],
    });
  }
}
...
let resolvedInvite;
function JoinGuild() {
  const guild = useStateFromStores([GuildStore], () =>
    GuildStore.getGuild("864267123694370836"),
  );
  ...
  const guildInfo = React.useMemo(() => {
    ...
    return {
      ...
      members: invite
        ...
        : GuildMemberCountStore.getMemberCount("864267123694370836"),
      online: invite
    ...
    children: [
      ...
      React.createElement(Button, {
        ...
        children: [
          ...
          !guild &&
            invite &&
            i18n.intl.format(i18n.t.QD7BDA, { guildName: guildInfo.name }),
...
function ErrorModal({ src, errorInfo }) {
  const highlighted = React.useMemo(
    () => hljs.highlight(errorInfo.stack, { language: "js" }),
    [],
  );

  const [copied, setCopied] = React.useState(false);

  const setNotCopied = React.useMemo(
    () => _.debounce(() => setCopied(false), 1000),
    [],
  );
...
function ErrorSplash({ src, errorInfo, windowKey }) {
  const Window = useStateFromStores([PopoutWindowStore], () =>
    PopoutWindowStore.getWindow(windowKey),
  );
  ...
  return React.createElement(React.Fragment, {
    children: [
                    ...
                    children: React.createElement("div", {
                      children: [
                        React.createElement(
                          ...
                          {},
                          i18n.intl
                            .string(i18n.t["8JQPPj"])
                            .replace("Discord", "Better Media Player"),
                        ),
                        React.createElement(
                          "p",
                          {},
                          i18n.intl
                            .string(i18n.t.tx8CkJ)
                            .replace("Discord", "Better Media Player"),
                        ),
                      ],
                    }),
      ...
      React.createElement("div", {
        ...
        onClick: () =>
          UI.alert(
            generateTitle(errorInfo),
            React.createElement(ErrorModal, { src, errorInfo }),
          ),
        children: React.createElement("svg", {
          viewBox: "0 0 24 24",
          height: 24,
      ...
      React.createElement("div", {
        ...
        children: [
          React.createElement("div", {
            ...
            children: [
              state === 0 &&
                React.createElement(Pause, { width: 24, height: 24 }),
              state === 1 &&
                React.createElement(Play, { width: 24, height: 24 }),
              state === 2 &&
                React.createElement(Replay, { width: 24, height: 24 }),
            ],
            onClick: () => {
              setPaused(!videoRef.current.paused);
              showControls(!videoRef.current.paused);
              ...
              onDrag: (multiplier) => {
                if (multiplier === 1) {
                  ...
                  videoRef.current.currentTime =
                    multiplier * videoRef.current.duration;

                setCurrentTime(videoRef.current.currentTime);

                if (durationBar.current)
                  durationBar.current.setGrabber(multiplier);
              },
              onDragEnd: () => {
...
const decodeBase64 = (str) => {
  return new TextDecoder().decode(
    Uint8Array.from(atob(str), (m) => m.codePointAt(0)),
  );
};
...
function PIPButton({ mediaRef }) {
  const [active, setActive] = React.useState(() => {
    return PopoutWindowStore.getWindowOpen(
      `DISCORD_PIP_${encodeBase64(mediaRef?.current?.src)}`,
    );
  });
...
const getMinWidth = () =>
  (!isNaN(MediaControls.minWidth) ? MediaControls.minWidth : 150) + 64;
  ...
  return [
    function useMinimumWidth() {
      const [minWidth, setMinWidth] = React.useState(
        () => Data.load("minimum-width") ?? getMinWidth(),
      );
  ...
  start() {
    ...
    Patcher.after(MediaControls.prototype, "componentDidMount", (that) => {
      ...
      that._BMP = addMinimumWidthListener(() => {
        const aspectRatioNode =
          that.mediaRef.current.parentElement.parentElement;
        ...
        if (!type && ret.type.prototype?.isReactComponent) {
          class Controls extends ret.type {
            render() {
              ...
              try {
                render.props.children.splice(
                  1,
                  0,
                  React.createElement(LoopButton, { mediaRef: that.mediaRef }),
                );
                render.props.children.splice(
                  -1,
                  0,
                  React.createElement(PIPButton, { mediaRef: that.mediaRef }),
                );
              } finally {
                return render;
              }
          ...
          children: [
            React.createElement(Text, {
              children: decodeBase64(
                props.windowKey.replace("DISCORD_PIP_", ""),
              )
                .split("?")[0]
                .split("/")
                .at(-1),
