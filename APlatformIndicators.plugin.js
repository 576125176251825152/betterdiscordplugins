  ...
  changelog: [
    {
      title: "Fixed",
      type: "fixed",
      items: ["Fixed for the latest Discord update"],
    },
    {
      title: "New Indicator",
      type: "added",
      items: ["Added support for VR"],
    },
  ],
  changelogDate: "2026-02-23",
};
...
const { useSyncExternalStore: useStateFromStoresObject } = Webpack.getByKeys(
...
const formatMessage = (key) =>
  Intl$1.intl.formatToMarkdownString(Intl$1.t[key]);
const Messages = {
...
function usePlatformStores(userId, type) {
  ...
  const clients = (() => {
    if (user?.id === UserStore.getCurrentUser()?.id) {
      if (sessions) {
        const clientStatuses = Object.entries(sessions).reduce(
          (acc, [, sessionData]) => {
...
function getStatusText(key, status) {
  return (
    ...
    ": " +
    Messages[
      `STATUS_${(status === "mobile" ? "mobile_online" : status).toUpperCase()}`
    ]
  );
}
...
function Checkbox({ checked, ...props }) {
  return checked
    ? React.createElement(CheckboxEnabled, {
        ...props,
      })
...
function SwitchItem(props) {
  const value = useStateFromStores([Settings], () =>
    Settings.get(props.id, props.value),
  );
  return React.createElement(
    SettingItem,
    {
...
function SmartDisable(props) {
  ...
  const [states, setStates] = React.useState(
    Settings.get(
      props.id,
      Object.fromEntries(props.items.map((item) => [item.id, item.value])),
    ),
  );
...
class PlatformIndicators {
  ...
  patchDMList() {
    ...
    const ChannelWrapper = Webpack.getBySource(
      "activities",
      "isMultiUserDM",
      "isMobile",
    );
    ...
    const ChannelWrapperElement = document.querySelector(
      `h2 + .${ChannelClasses.channel}`,
    );
    if (ChannelWrapperElement) {
      const ChannelWrapperInstance = ReactUtils.getOwnerInstance(
        ChannelWrapperElement,
      );
    ...
    Patcher.after(NameWrapper, "render", (_, __, res) => {
      ...
      if (!user) return;
      const child = Utils.findInTree(
        res,
        (e) => e?.className?.includes("nameAndDecorators"),
        {
  ...
  patchMemberList() {
    const [MemberItem, key] = Webpack.getWithKey(
      Webpack.Filters.byStrings("nameplate:", ".MEMBER_LIST"),
    );
    Patcher.after(MemberItem, key, (_, [props], ret) => {
      ...
      const child = findInReactTree(ret, (e) =>
        e?.className?.includes("username"),
      );
      if (user && child) {
        child.children = [
          child.children,
  ...
  patchChat() {
    const [ChatUsername, key] = Webpack.getWithKey(
      Webpack.Filters.byStrings(".guildMemberAvatar&&null!="),
    );
  ...
  patchBadges() {
    ...
    Patcher.after(BadgeList, Key_BL, (_, [{ displayProfile }], res) => {
      ...
      if (Settings.get("ignoreBots", true) && displayProfile?.application)
        return;
  ...
  patchFriendList() {
    const UserInfo = Webpack.getBySource(
      "user",
      "subText",
      ...
      (_, __, res) => {
        ...
        Patcher.after(res.type.prototype, "render", (_2, __2, res2) => {
          const unpatch2 = Patcher.after(res2, "type", (_3, __3, res3) => {
            ...
            const child = Utils.findInTree(
              res3,
              (e) => e?.className?.includes("listItemContents"),
              {
                walkable: ["children", "props"],
              },
            );
