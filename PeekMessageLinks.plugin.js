...
const Message = Webpack.getModule((m) =>
  String(m.type).includes('Nt,"aria-setsize":-1'),
);
...
const updateMessageReferenceStore = (() => {
  function getActionHandler() {
    ...
    const storeHandlers = Object.values(nodes).find(
      ({ name }) => name === "ReferencedMessageStore",
    );
  ...
  changelog: [
    {
      title: "Fixed",
      type: "fixed",
      items: ["Fixes for discord updates."],
    },
  ],
  settings: [
    {
      ...
      value:
        BdApi.Data.load("PeekMessageLinks", "settings")?.onClick ?? "popup",
      options: [
    ...
    },
    {
      ...
      value:
        BdApi.Data.load("PeekMessageLinks", "settings")?.onShiftClick ??
        "navigate",
      options: [
        { label: "Show in popup", value: "popup" },
        { label: "Navigate to message", value: "navigate" },
    ...
    },
    {
      ...
      value:
        BdApi.Data.load("PeekMessageLinks", "settings")?.onCtrlClick ?? "none",
      options: [
        { label: "Show in popup", value: "popup" },
        { label: "Navigate to message", value: "navigate" },
        { label: "Do nothing", value: "none" },
...
module.exports = class PeekMessageLinks {
  ...
  loadSettings() {
    return {
      ...this.defaultSettings,
      ...BdApi.Data.load("PeekMessageLinks", "settings"),
    };
  }
  ...
  inMana(node) {
    let item = BdApi.ReactUtils.getInternalInstance(
      document.querySelector("div[class^=app_] > div[class^=app_]"),
    );
  ...
  patchChannelMention() {
    const ChannelMentionBubble = Webpack.getModule(
      (m) => m.defaultRules && m.parse,
    ).defaultRules.channelMention;
    Patcher.after(
      "PeekMessageLinks-ChannelMentionBubble",
      ChannelMentionBubble,
      "react",
      (_, [props], res) => {
  ...
  async handleAction(action, props, targetElement, originalClick, event) {
    ...
    if (!props.messageId) {
      let messageStore = Webpack.Stores.MessageStore.getMessages(
        props.channelId,
      );
      ...
      if (!message) {
        try {
          const messagePromise =
            this.messageCache.get(props.messageId) ||
          ...
          if (message.id !== props.messageId) {
            message = new MessageConstructor({
              ...
              flags: 64,
              content:
                "This message has likely been deleted as the returned message ID does not match that of the message link. If you attempt to navigate to the message, you will be redirected to the closest message.",
      ...
      if (message.messageReference) {
        if (
          ...
          let referencedMessage = MessageStore.getMessage(
  ...
  showMessagePopup(messages, targetRect, channelId) {
    ...
    const channel = ChannelStore.getChannel(
      channelId || (messages[0] && messages[0].channel_id),
    );
    ...
    const PopupComponent = () => {
      ...
      const adjustedMaxHeight = Math.min(
        maxHeight,
        showBelow
          ? spaceBelow - buffer - edgeMargin
          : spaceAbove - buffer - edgeMargin,
      );
      ...
      if (
        window.innerHeight - bottomPosition - adjustedMaxHeight <
        edgeMargin
      ) {
