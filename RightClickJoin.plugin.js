...
module.exports = class RightClickJoin {
  ...
  load() {
    try {
      ...
      console.error(
        config.info.name,
        "Failed to reach the ZeresPluginLibrary for Plugin Updater.",
        err,
      );
    }
  }
  ...
  start() {
    try {
      ...
      this.selectVoiceChannel = Webpack.getModule(
        Filters.byKeys("selectChannel"),
      ).selectVoiceChannel;
  ...
  patchUserContextMenu() {
    config.userContextPatch = ContextMenu.patch(
      "user-context",
      (returnValue, props) => {
        ...
        let callButtonParent = Utils.findInTree(
          returnValue,
          (e) => Array.isArray(e) && e.some(callButtonFilter),
        );

        if (Array.isArray(callButtonParent))
          if (
            !callButtonParent.some(
              (button) => button?.props?.id === config.info.slug,
            )
          )
            // Splice in our own logic above the call-button
            callButtonParent.splice(
              callButtonParent.findIndex(callButtonFilter),
              0,
              this.rightClickJoinMagic(props.user),
            );
      },
    );
  }
  ...
  rightClickJoinMagic(user) {
    const voiceStateForUser = this.VoiceStateStore.getVoiceStateForUser(
        user.id,
      ),
    ...
    if (!voiceStateForUser) {
      if (
        this.UserProfileStore.isFetchingProfile(user.id) ||
...
const UserFetchActions = new (class UserFetchActions {
  ...
  unwrapApply(variable) {
    const regex = new RegExp(
      String.raw`function (\w+)\([\w,]+\)\{return ${variable}.apply\(this,arguments\)\}`,
    );
  ...
  parseExports() {
    const fetchProfileRegex =
      /function (\w+)\(\)\{\w=[\s\S]+?switch\(\w\.label\)\{[\s\S]+?type\:\"USER_PROFILE_FETCH_START\",userId\:/;
    ...
    if (fetchProfileRegex.test(this._moduleString)) {
      ...
      this._nativeFetchProfile =
        this._module[this.findExportByVariable(unwrapped)];
    }

    const fetchFriendsRegex =
      /function (\w+)\(\)\{return\(\w=.+?switch\(\w\.label\)\{[\S\s]+?type:"MUTUAL_FRIENDS_FETCH_START",userId:/;

    if (fetchFriendsRegex.test(this._moduleString)) {
      const variable = fetchFriendsRegex.exec(this._moduleString)?.[1];
      const unwrapped = this.unwrapApply(variable);

      this._nativeFetchFriends =
        this._module[this.findExportByVariable(unwrapped)];
    }
  }
  ...
  initialize() {
    ...
    this._module = Webpack.getModule((m, _t, id) =>
      (this._moduleString = Webpack.modules[id]?.toString()).includes(
