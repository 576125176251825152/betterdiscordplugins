...
const defaultAudioUrl =
  "https://raw.githubusercontent.com/nicola02nb/BetterDiscord-Stuff/main/Plugins/NotifyWhenMuted/stop_talking.wav";
const config = {
  changelog: [
  ...
  settings: [
    {
      type: "switch",
      id: "enabled",
      name: "Enable Notify When Muted",
      note: "Enables/Disables the plugin audio notifications.",
      value: true,
      settings: [],
    },
    {
      type: "switch",
      id: "notifyWhenDeafen",
      name: "Notify When Deafen",
      note: "Notify when you are self deafen.",
      value: true,
    },
    {
      type: "switch",
      id: "notifyServerMuted",
      name: "Notify When Server Muted",
      note: "Notify when you get muted by server.",
      value: false,
    },
    {
      type: "text",
      id: "audioUrl",
      name: "Custom Audio URL",
      note: "URL to the audio file to play when user tries to speak while muted.",
    ...
    {
      type: "number",
      id: "delayBetweenNotifications",
      name: "Delay Between Audio Notifications (ms)",
      note: "Delay Between Audio Notifications in milliseconds.",
      value: 5000,
    },
    {
      type: "switch",
      ...
      note: "Displays a button nearby krisp in the RTC panel to toggle On/Off the audio notifications.",
      value: true,
    },
  ],
};
  ...
  return config.settings.reduce(
    (found, setting) =>
      found
        ...
        : setting.id === key
          ? setting
          : setting.settings?.find((s) => s.id === key),
    undefined,
  );
}
...
const voiceButtonsContainer = [
  ...Webpack.getWithKey(Filters.byStrings("NO_WRAP", "flexShrink:"), {
    searchExports: true,
  }),
];
...
module.exports = class NotifyWhenMuted {
    ...
    settings.forEach((setting) => {
      if (setting.settings) {
        ...
        this.settings[setting.id] =
          Data.load(this.meta.name, setting.id) ?? setting.value;
      }
    });
  }
  ...
  start() {
    ...
    DOM.addStyle(
      this.meta.name,
      `.toggleNotifyMuted > svg {transition: transform 1s ease-in-out;}
    ...
    Patcher.after(
      this.meta.name,
      voiceButtonsContainer[0],
      voiceButtonsContainer[1],
      this.addButton,
    );
  }
  ...
  async handleSpeaking(_, args, ret) {
    if (
      ...
      (!this.settings.notifyServerMuted &&
        !MediaEngineStore.isSelfMute() &&
        MediaEngineStore.isMute()) ||
      (MediaEngineStore.isSelfDeaf() && !this.settings.notifyWhenDeafen)
    )
      return;
    ...
    const getIcon = (enabled) => {
      return React.createElement(
        ...
        React.createElement(
          ...
          {
            transform:
              "translate(4.7,3.3) scale(1,1) translate(-4.7,-3.3) translate(4.7,11.4) matrix(0.5,0,0,0.5,-0.8,-8)",
          },
          React.createElement("path", {
        ...
        React.createElement(
          "button",
          {
            ...
            className:
              ...
              buttonStates.button +
              " " +
              buttonStates.enabled +
              " " +
              buttonLook.button +
              " " +
              buttonLook.lookBlank +
              " " +
              buttonLook.colorBrand +
              " " +
              buttonLook.grow,
            onClick: () => {
              update();
